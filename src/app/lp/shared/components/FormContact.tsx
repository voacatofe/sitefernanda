...
export default function FormContact({ empreendimento, whatsappNumber }: FormContactProps) {
  ...
  return (
    <Card className="max-w-2xl mx-auto border border-border/30">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleChange}
              rows={4}
              className="w-full px-4 py-2 rounded-md border border-border/30 bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-fernanda-gold/50 resize-none"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Button
              type="submit"
              className="flex-1 bg-fernanda-gold hover:bg-fernanda-gold/90 text-dimas-black"
            >
              Enviar Mensagem
            </Button>
            ...
            <Button
              type="button"
              variant="outline"
              className="flex-1 border-fernanda-gold text-fernanda-gold hover:bg-fernanda-gold/10"
              asChild
            >
              <Link
                href={`https://wa.me/${whatsappNumber}?text=Olá%20Fernanda,%20gostaria%20de%20mais%20informações%20sobre%20o%20${empreendimento}.`}
                target="_blank"
              >
                <svg 
